...
class CSheet(object):
	def __init__(self,name,values):
		if type(name) == dict:
			self.xls_filename = name.get("xls_filename")
			self.name = name.get("sheet_name")
		else:
			self.xls_filename = ""
			self.name = name
		self.showname = "%s#%s" % (self.xls_filename,self.name)
		if sys.platform == "win32":
			self.showname = self.showname.encode("gbk")
		else:
			self.showname = self.showname.encode("utf-8")
	...
	def value(self,row,col):
		...
		if parser:
			try:
				...
				errmsg = "ERROR %s<%d,%d>: %s" % (self.showname,row,col,e.message)
 				print(errmsg)
...
class CParser(object):
	...
	def linefmt(self,linefmt,line,row):
		import re
		pat = "%\(([^()]+)\)([-+0]?[1-9]?[.]?[1-9]?[fdsr])"	# 提取格式字段 "%[(name)][flags][width].[precision]typecode" :如%(name)fmt => (name,fmt)
		...
		#print "linefmt",lst
		if s:
			errmsg = "ERROR %s<%s,%s>:unknow format:%s" % (self.m_sheet.showname,row,None,s)
			print(errmsg)
			raise Exception(errmsg)
		#对None值的默认处理
		for name, fmt in lst:
			_value = self.cellfmt(name, fmt, line[name],row)
			if _value != None:
				line[name] = _value
			else:
				if line[name] == None:
					if "d" in fmt:
						...
						if self.m_cfg.get("none2nil"):
							...
							line[name] = ""
				elif type(line[name]) == float:
					# 3.0 => 3
					if line[name] == int(line[name]):
						line[name] = int(line[name])
		try:
			aline = linefmt % line
		except Exception,e:
			errmsg = "Error %s<%s,%s>: %s" % (self.m_sheet.showname,row,str(e) + "\n" + str(line))
			print(errmsg)
			raise Exception(errmsg)
		return aline

	# 可以用CSheet.register_parser代替
	def cellfmt(self, name, fmt, value,row):
		return None
...
class CMultiLineParser(CParser):
	def parse(self):
		startrow = self.m_cfg.get("ignorerows",0)
		linefmt = self.m_cfg.get("linefmt","%s")
		idkey = self.m_cfg.get("multilineid","id")
		multilinefmt = self.m_cfg.get("multilinefmt","%s")
		...
		for row in xrange(startrow,self.m_sheet.rows()):
			line = self.m_sheet.line(row)
			line2 = self.linefmt(multilinefmt,line,row)
			#print(lastid,line[idkey],firstline)
			if lastid == None or (line[idkey] != None and lastid != line[idkey]):
				if firstline != None:
					multiline = "\n".join(multilines)
					firstline["multiline"] = multiline
					#firstline["multilinefmt"] = multiline
...
def daobiao(sheet,name,cfg,dstpath,myparser=None):
